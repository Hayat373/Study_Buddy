...
@section('title', 'Edit Schedule - Study Buddy')
...
<style>
    ...
    .recurring-options {
        ...
        border-radius: 12px;
        border: 1px solid rgba(57, 183, 255, 0.1);
        display: {{ $schedule->recurring ? 'block' : 'none' }};
    }
...
<div class="dashboard-container">
    <div class="dashboard-header">
        <h1>Edit Schedule</h1>
        <p>Update your study session, exam, or reminder</p>
    </div>
        ...
        <form action="{{ route('schedule.update', $schedule->id) }}" method="POST">
            ...
            <div class="form-group">
                ...
                <select class="form-control form-select" id="type" name="type" required>
                    <option value="study_session" {{ $schedule->type == 'study_session' ? 'selected' : '' }}>Study Session</option>
                    ...
                    <option value="group_study" {{ $schedule->type == 'group_study' ? 'selected' : '' }}>Group Study</option>
                </select>
            </div>
            
            <div class="form-group">
                <label class="form-label">Color</label>
                <input type="hidden" id="color" name="color" value="{{ $schedule->color }}">
                <div class="color-options">
                    <div class="color-option {{ $schedule->color == '#2dc2ff' ? 'active' : '' }}" style="background-color: #2dc2ff;" data-color="#2dc2ff"></div>
                    <div class="color-option {{ $schedule->color == '#78f7d1' ? 'active' : '' }}" style="background-color: #78f7d1;" data-color="#78f7d1"></div>
                    <div class="color-option {{ $schedule->color == '#ff6b6b' ? 'active' : '' }}" style="background-color: #ff6b6b;" data-color="#ff6b6b"></div>
                    <div class="color-option {{ $schedule->color == '#ffa726' ? 'active' : '' }}" style="background-color: #ffa726;" data-color="#ffa726"></div>
                    <div class="color-option {{ $schedule->color == '#ab47bc' ? 'active' : '' }}" style="background-color: #ab47bc;" data-color="#ab47bc"></div>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="datetime-local" class="form-control" id="start_time" name="start_time" 
                    value="{{ $schedule->start_time->format('Y-m-d\TH:i') }}" required>
            </div>
            
            <div class="form-group">
                <label class="form-label" for="end_time">End Time</label>
                <input type="datetime-local" class="form-control" id="end_time" name="end_time" 
                    value="{{ $schedule->end_time->format('Y-m-d\TH:i') }}" required>
            </div>
            
            <div class="form-group">
                <label class="form-label">
                    <input type="checkbox" id="recurring" name="recurring" value="1" {{ $schedule->recurring ? 'checked' : '' }}> Recurring Event
                </label>
                ...
                <div class="recurring-options" id="recurringOptions">
                    <div class="form-group">
                        ...
                        <select class="form-control form-select" id="recurring_pattern" name="recurring_pattern">
                            <option value="daily" {{ $schedule->recurring_pattern == 'daily' ? 'selected' : '' }}>Daily</option>
                            <option value="weekly" {{ $schedule->recurring_pattern == 'weekly' ? 'selected' : '' }}>Weekly</option>
                            <option value="monthly" {{ $schedule->recurring_pattern == 'monthly' ? 'selected' : '' }}>Monthly</option>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" class="form-control" id="recurring_until" name="recurring_until" 
                            value="{{ $schedule->recurring_until ? $schedule->recurring_until->format('Y-m-d') : '' }}">
                    </div>
                </div>
            </div>
